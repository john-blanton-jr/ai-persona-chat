...
async def get_chat_history(user_id: str, persona_id: str):
    ...
    if history:
        ...
        serialized_history = serializeList(history)
        print("serialized HISTORY!!!!!!!!", serialized_history)
...
@chat.get("/get_all_personas")
async def get_all_personas():
    try:
        # Fetch all personas from the MongoDB collection
        all_personas = list(conn.local.personas.find())

        # Serialize the list of personas using your custom function
        serialized_personas = serializeList(all_personas)

        return JSONResponse(content={"personas": serialized_personas})
    except Exception as e:
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="Internal Server Error")
